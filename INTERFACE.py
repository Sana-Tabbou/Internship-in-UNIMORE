...
class AudioGUI:
    def __init__(self, master):
        ...
        self.lang_menu = tb.Combobox(lang_frame, values=["English", "Italien"], width=10)
    ...
    def ask_cutoff(self):
        try:
            return float(simpledialog.askstring(self.text["cutoff_prompt"], "", parent=self.master))
    ...
    def classify_audio(self):
        if self.current_file is None:
            messagebox.showwarning("⚠️", self.text["no_file"])
            return
        label = simpledialog.askstring(self.text["classification_prompt"], "", parent=self.master)
        if label is None:  # If the user cancel 
            return
